...
public class Bluetooth {
    ...
    private class ConnectedThread extends Thread {
        ...
        private void PosttoApi(String message) {
            try {
                HttpClient httpclient = new DefaultHttpClient();
                // make GET request to the given URL
               HttpResponse httpResponse = httpclient.execute(new HttpGet("http://robochimera.azurewebsites.net/api/values/"+message));
            } catch (Exception e) {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    PosttoApi(new String(buffer));
                    // Send the obtained bytes to the UI Activity
                    mHandler.obtainMessage(MESSAGE_READ, bytes,
                            -1, new String(buffer)).sendToTarget();
                } catch (IOException e) {
    ...
    public void sendMessage(String message, Context ctx) {
        ...
        if (message.length() > 0) {
            ...
            Toast.makeText(ctx, "Message before send  - " + message.toString(), Toast.LENGTH_LONG).show();
            this.write(send);
        }
    }
